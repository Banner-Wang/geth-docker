...
# pip3 install aiohttp==3.8.6 click==8.0.4 dingtalkchatbot==1.5.7 
...
async def send_dingding_alert(token: str, content: Dict) -> None:
    """Send alert to DingDing"""
    try:
        if not token:
            logger.warning("DingDing token not provided, skipping alert")
            return
        ...
        final_msg = "\n".join(msg_parts)
        # print(final_msg)
        await run_async_send(bot, final_msg)
        logger.info("DingDing alert sent successfully")
        
    except Exception as e:
        logger.error(f"Failed to send DingDing alert: {str(e)}")

async def run_async_send(bot, msg: str) -> None:
    """Async send DingDing message"""
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, bot.send_text, msg)
...
@click.option('--dingding-token', default='',
              help='DingDing robot access token for alerts')
def check_health(root_path: str, json_output: bool, dingding_token: str):
    ...
    # Send DingDing alert
    if dingding_token:
        run_async(send_dingding_alert(dingding_token, result))
